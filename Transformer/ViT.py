...
class vision_transformer(nn.Module):
    ...
    def __init__(self, in_channels : int = 3, img_size : int = 224, patch_size : int = 16, emb_dim : int = 16 * 16 * 3,
        ...
        self.image_embbeding = image_embedding(in_channels, img_size, patch_size, emb_dim)
    ...
    def forward(self, x):
        x = self.image_embbeding(x)
        attentions = []
        for block in self.encoder_module:
            x, attention = block(x)
...
y, att = vision_transformer(3, 224, 16, 3 * 16 * 16, 15, 3, 4, 0.2, 10)(x)
